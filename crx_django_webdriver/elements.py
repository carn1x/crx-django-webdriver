...
class DjangoWebdriverElement(object):
    ...
    def __init__(self, selector, select_method, testcase, name=None):
        self.selector = selector
        self.select_method = select_method
        self.testcase = testcase
        self.name = name
        self.find_displayed = False
        time.sleep(settings.DJANGO_WEBDRIVER_PER_COMMAND_DELAY)
    ...
    def select(self, override_selector=None, override_method=None):
        ...
        if not self.find_displayed:
            return self.testcase.driver.find_element(by=by, value=selector)
        else:
            elements = self.testcase.driver.find_elements(by=by, value=selector)
    ...
    def assert_(self, assertions, values=None, find_displayed=False):
        ...
        for assertion in assertions:
            if assertion == a.WAIT:
                ...
                self.testcase.assertRegexpMatches(self.select_quick().get_attribute('innerText'), values[assertion])

            elif assertion == a.CONTAINS:
                self.testcase.assertIn(values[assertion], self.select_quick().get_attribute('innerText'))

            elif assertion == a.NOT_CONTAINS:
                self.testcase.assertNotIn(values[assertion], self.select_quick().get_attribute('innerText'))
                ...
                self.testcase.assertTrue(int(self.select_quick().get_attribute('innerText')) >= values[assertion])
                ...
                try:
                    WebDriverWait(self.testcase.driver, settings.DJANGO_WEBDRIVER_IMPLICITLY_WAIT).until(
                ...
                raise Exception('AssertElement unknown assertion: {}'.format(assertion))
    ...
    def select_quick(self):
        ...
        """
        self.testcase.driver.implicitly_wait(settings.DJANGO_WEBDRIVER_IMPLICITLY_WAIT_QUICK_ASSERT)
        select = None
        ...
        self.testcase.driver.implicitly_wait(settings.DJANGO_WEBDRIVER_IMPLICITLY_WAIT)
        return select
    ...
    def capture(self, pattern=None, result_index=None, _property=None, _property_key=None):
        ...
        _property = _property or 'innerText'
        ...
        if m and m.group(result_index):
            ...
            raise Exception('ReturnElement can not find "{}" in "{}"'.format(pattern, text))
    ...
    def hover(self, value=None):
        ActionChains(self.testcase.driver).move_to_element(self.select()).perform()

    def hover_click(self, value=None):
        ActionChains(self.testcase.driver).move_to_element(self.select()).click().perform()
    ...
    def drag_to(self, destination):
        ...
        if offset_y < 0:
            ...
            offset_y += 1
        ActionChains(self.testcase.driver).drag_and_drop_by_offset(source, offset_x, offset_y).perform()

    def text(self, value=None):
        return self.select_quick().get_attribute('innerText')
